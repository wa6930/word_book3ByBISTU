...
public class Function {
    ...
    public static List<NewsItemModel> parseHtmlData(String result) {
        ...
        String toSelect="<img src=([^></a>]*)></a> </div>          <div class=\"span10\">\n" +
                "                <h4><a target=\"_blank\"  href=\"([^\"]*)\">([^</a>]*)</a></h4>\n" +
                "                <p>([^<br>]*)<br>\n" +
                "                  <smaill>([^</smaill>]*)</smaill>";//用于爬取
...
//                        "<h4><a target=\"_blank\"  href=\"([^\"]*)\">([^</a>]*)</a></h4><p>([^<br>]*)<br><smaill>([^</smaill>]*)</smaill></p>");
        ...
        while (matcher.find()) {
            ...
            model.setNewsDetailUrl(matcher.group(2).trim());
            ...
            model.setNewsOrigin(matcher.group(5).trim());

            sb.append("详情页地址：" + matcher.group(2).trim() + "\n");
            sb.append("图片地址：" + matcher.group(1).trim() + "\n");
            sb.append("标题：" + matcher.group(3).trim() + "\n");
            sb.append("概要：" + matcher.group(4).trim() + "\n");
            sb.append("来源："+matcher.group(5).trim()+"\n\n");
